...
const expirationDateValidation = (value: string) => {
	return /^\d{2}\/\d{2}$/.test(value)
		? undefined
		: 'You need to follow the format MM/YY';
};
...
function PaymentSelectionForm({ className, paymentData, onSubmit }: Props) {
	...
	return (
		<section className={cx(styles.paymentSelectionForm, className)}>
			<h2 className={styles.paymentSelectionFormTitle}>
				CREDIT/DEBIT CARD
			</h2>
			...
			<p className={styles.paymentSelectionFormTerms}>
				By clicking Place Order you agree to the{' '}
				<a className={styles.paymentSelectionFormTermsLink} href="#">
					Terms & Conditions
				</a>{' '}
				and{' '}
				<a className={styles.paymentSelectionFormTermsLink} href="#">
