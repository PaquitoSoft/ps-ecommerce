...
function ProductImagesCarousel({ product, className }: Props) {
	...
	const [{ carouselPosition, carouselIndex }, setCarouselData] = useState({
		carouselPosition: 0,
		carouselIndex: 0,
	});
		...
		carouselItems = [
			product.detailImages.map((imageUrl) => (
				<li
					...
					style={{ width: $container.current?.clientWidth || 0 }}
				>
					<ProductImage
						imageUrl={imageUrl}
						alt={product.altName}
	...
	return (
		<div
			className={cx(styles.productImagesCarousel, className)}
			ref={$container}
		>
			<ul
			...
			{carouselIndex !== 0 && (
				<IconButton
					...
					onClick={() => moveCarousel(carouselIndex - 1)}
				>
					<ArrowLeftIcon />
				</IconButton>
			)}
