	...
	return (
		<nav className={styles.gridPaginator}>
			...
			<PaginatorLink
				categoryCode={categoryCode}
				pageNumber={currentPage + 1}
				isVisible={currentPage !== totalPagesCount}
			>
				NEXT
			</PaginatorLink>
		</nav>
...
function ProductsGrid({ category, products, totalProductsCount }: PageProps) {
	const {
		state: { pageNumber, categoryCode, totalPagesCount },
		actions: { isProductInWishlist, updateProductInWishlist },
	} = useProductsGrid({
		totalProductsCount,
	...
	return (
		<div>
			<div>
				...
				<span className={styles.totalProductsCount}>
					[{totalProductsCount}]
				</span>
			</div>
				...
				{products.map((product) => (
					<li key={product.code}>
						<ProductSummary
							...
							categoryCode={categoryCode}
							showColors={true}
							isWishlistedProduct={isProductInWishlist(
								product.id
							)}
...
export const getServerSideProps: GetServerSideProps = async (context) => {
	...
	return {
		props: {
			...
			totalProductsCount:
				apolloQueryData.data.productsByCategory.totalCount,
		},
	};
};
